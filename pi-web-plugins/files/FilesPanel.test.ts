...
describe("Files panel component boundary", () => {
  ...
  it("collapses and expands the file tree while keeping the file viewer available", async () => {
    const listFiles = vi.fn<WorkspaceFilesCapabilityV1["listFiles"]>(() => Promise.resolve(treeResponse("", [{ name: "README.md", path: "README.md", type: "file" }])));
    ...
    expect(panel.shadowRoot?.querySelector("pi-web-files-viewer")).not.toBeNull();
